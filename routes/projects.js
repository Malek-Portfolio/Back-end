...
router.get("/", async (req, res) => {
  try {
    const projects = await Projects.find(
      {},
      { thumbnail: 1, title: 1, description: 1, link: 1, type: 1, category: 1  ,slug : 1 }
    );
    res.json(projects);
  } catch (err) {
...
router.get("/:slug" , async (req , res) => {
    ...
    try {
        const projects = await Projects.findOne(
          {slug : slug},
          { thumbnail: 1, title: 1, description: 1, link: 1, type: 1, category: 1  ,slug : 1 , features: 1 , technologies : 1 }
        );
        res.json(projects);
      } catch (err) {
        res.status(500).json({ message: err.message });
